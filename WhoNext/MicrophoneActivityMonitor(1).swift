...
class MicrophoneActivityMonitor: ObservableObject {
    ...
    // Track if we've logged the accessibility warning
    ...
    /// Note: This requires Accessibility permissions. If denied, falls back to basic app detection.
    private func checkWindowTitleForCallKeywords(bundleID: String) -> Bool {
        ...
        // Try to use Accessibility API for better detection
        // This may fail due to sandbox restrictions - that's OK, we'll fall back
        let axApp = AXUIElementCreateApplication(app.processIdentifier)
        var windowList: CFTypeRef?

        let result = AXUIElementCopyAttributeValue(axApp, kAXWindowsAttribute as CFString, &windowList)

        // Handle sandbox denial or lack of permissions gracefully
        ...
        guard result == .success,
            ...
            // Any other error: fall back to basic detection
            return app.isActive
        }
        ...
        // No call keywords found, but if app is frontmost, might still be in a call
        // (window title may not contain keywords)
        return false
    }
}
